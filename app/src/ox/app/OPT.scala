...
/** <p> {{{Bernard Sufrin, Oxford, 2015... $}}}
  ...
  *       REST(
  *         "--",
  *         { case args => for (f <- args) jobs.enqueue((env, f)) },
...
object OPT {
  ...
  def OPT(tag: String, flag: Boolean, value: Boolean, help: String): Opt =
    macro BoolArg
  def BoolArg(
      c: blackbox.Context
  ...
  /** An non-option argument (ie one that doesn't start with "-") invokes
    ...
    * human-readable description of the non-option argument pattern. (I often
    * use '"<path>"' here).
    */
