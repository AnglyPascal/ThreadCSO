...
/** A synchronized channel to be used by at most one reader and writer process
  * ''simultaneously''. There are dynamic checks for ''simultaneous'' multiple
  * reads (writes), but they are are ''incomplete''. They cannot reliably detect
  * that multiple readers (writers) are running concurrently with a single
  * writer (reader) that is fast enough to keep up with their read (write) rate.
  *
  * The channel closes when either ``closeOut()`` or ``closeIn()`` or
  * ``close()`` is invoked.
  *
  * @param name_
  *   the name of the channel, as reported by the debugger
...
class OneOne[T](name_ : String) extends SyncChan[T] {
  ...
  /** READY if not closed and there's a writer waiting to sync (else CLOSED or
    * UNKNOWN) (used only by alternation implementations)
    */
  ...
  /** READY if not closed and there's a reader waiting and last writer has
    * already synced (else CLOSED or UNKNOWN) (used only by alternation
    * implementations)
    */
  ...
  def !(value: T) = {
    ...
    assert(
      lastWriter == null,
      s"!($value) in ${current.identity} overtaking !($buffer) [${lastWriter.identity}]"
    )
    buffer = value
    full.set(true)
  ...
  def ?(): T = {
    ...
    assert(
      lastReader == null,
      s"?() overtaking [${lastReader.identity}] in ${current.identity}"
    )
    outPortEvent(READYSTATE) // Announce state change to any alternation
    while (!closed.get && !full.get) // AWAIT BUFFER FROM WRITER
      {
  ...
  def readBefore(timeoutNS: Long): Option[T] = {
    assert(
      reader.get == null,
      s"?() overtaking [${reader.get.identity}] in ${Thread.currentThread.identity}"
    )
    checkOpen
    val current = Thread.currentThread
    reader.set(current)
    outPortEvent(READYSTATE)
    val success =
      0 < parkUntilElapsedOr(this, timeoutNS, closed.get || full.get)
    checkOpen
    val result = buffer
