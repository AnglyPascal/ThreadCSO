...
object CSOThreads {
  ...
  /** PRE: size of CSO platform thread pools (or count of CSO threads) is not
    * increasing
    *
  ...
  /** PRE: size of CSO virtual thread pool is not increasing
    ...
    */
  def forActiveVThreads(f: Thread => Unit) = vThreads.valuesIterator.foreach(f)
  ...
  /** Setting a jdk property with `-Dio.threadcso.pool=`''secs'' specifies that
    * idling pooled threads expire after ''secs'' idling.
    *
    * (deprecated: default 4)
    */
  @deprecated
  val poolSecs: Long = getPropElse("io.threadcso.pool", _.toLong)(4L)
  ...
  /** Setting a jdk property with `-Dio.threadcso.pool.MAX=`''number'' specifies
    ...
    * pools.
    *
    * Leave it at its default value unless you have understood the documentation
  ...
  /** This executor keeps track of the currently-running virtual threads, at a
    * slight cost in their startup and closedown times.
    */
  ...
  /** This executor doesn't keep track of the currently-running virtual threads.
    * They start a mite faster than VIRTUAL threads but the debugger doesn't get
    * to see them
    */
  ...
  /** Setting a jdk property with `-Dio.threadcso.pool.KIND=`''kind'' determines
    * what kind of thread pooling to use by default:
    *
    *   - `ADAPTIVE` -- Retires idle threads after `poolSECS`. See
    ...
    *
    *   - `VIRTUAL` -- makes a new virtual thread to start and run a `PROC`.
    *     Stacksize is as specified by the `PROC`.
    *
    *   - `FASTVIRTUAL` -- makes a new virtual thread to start and run a `PROC`.
    *     Stacksize is as specified by the `PROC`. The thread is not visible to
    *     the debugger.
    *
    * Default is VIRTUAL
    */
  ...
  /** The current default `CSOExecutor` used in this program. Its kind is
    * determined by `poolKIND`.
    */
  def executor(): CSOExecutor = getExecutor(poolKIND)

  private def getExecutor(poolKind: String): CSOExecutor =
  ...
  /** Shut all executors down.
    *
    * TODO: This could be somewhat more efficient. (Only shut down the used
    * executors) It's only used to debrief the pooled executors.
    */
  def shutDown(): Unit = {
