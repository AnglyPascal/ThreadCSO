...
object Process {
  ...
  private val processCount = new java.util.concurrent.atomic.AtomicLong

  /** Generate a new process name */
  def genName: String = "Proc-%d".format(processCount.getAndIncrement)

  /** This is invoked to handle exceptional non-`Stopped` termination of a
  ...
  class Par(_name: String)(procs: collection.Seq[PROC]) extends PROC {
    ...
    /** Run each component process in a separate thread. The first runs in the
      * current thread; the others are given threads of their own.
      */
    def apply(): Unit = {
      ...
      // ------------- Termination state --------------
      {
        ...
        for (handle <- peerHandles) {
          (thrown, handle.thrown) match {
            ...
            case (_, _) =>
              throw ParException(
                List(firstHandle.thrown) ++ peerHandles.map(_.thrown)
              )
          }
        }
  ...
  /** A `ParException` is thrown when one or more components of a running
    * parallel composition terminate with non-`Stopped` exceptions:
  ...
  case class ParException(throwables: collection.Seq[Throwable])
    ...
    override def toString: String =
      "ParException(%s)".format(throwables.mkString(", "))
  }
  ...
  class Handle(
    ...
    override def toString =
      s"Handle($name, ..., $latch, $stackSize) thrown=$thrown; thread=$thread"
    ...
    /** Acquire a thread (with the advisory `stackSize`) and start `body`
    ...
    override def run(): Unit = {
      ...
      try {
        ...
        case other: Throwable =>
          if (Process.handleThrowable != null)
            Process.handleThrowable(name, other)
          this.thrown = other
      } finally {
